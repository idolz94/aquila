$(function() {
            ...
            if (dataAdmin == 1) {
                $('td', row).eq(2).empty().append(`
                    <div>
                        ...
                        <a type="submit" class="btn btn-secondary delete-trigger text-white">
    ...
    $('body').on('click', '.delete-trigger', function(event) {
        ...
        $('#delete-lydo-form').attr('action', action);
    });

    $('body').on('click', '#modal-danger .yes-confirm', e => {
        $('#delete-lydo-form').submit();
    });
    ...
    // submit form create
    $('body').on('submit', '#create_lydo', function(e) {
        e.preventDefault();
        let urlIndex = $(this).data('url-index');
